...
const api = axios.create({
  baseURL: "https://basic-auth-app-api-with-django-m0hamadys-projects.vercel.app/api", // Replace with your backend URL
});
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response && (error.response.status === 401 || error.response.status === 403)) {
      ...
      if (refreshToken) {
        try {
          const response = await axios.post("https://basic-auth-app-api-with-django-m0hamadys-projects.vercel.app/api/auth/users/token/refresh/", {
